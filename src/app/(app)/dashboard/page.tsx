...
function getHealthStatus(h: NumberHealth): {
    ...
    // Need at least 5 calls to judge health
  if (h.total < 5) {
        return {
                label: "New",
                color: "text-muted",
                icon: "green",
                reason: "Not enough call history yet",
        };
  }

  // Use recent (7d) rate if available, otherwise all-time
  ...
  if (answerRate >= 35) {
        return {
                ...
                color: "text-success",
                icon: "green",
                reason: `${Math.round(answerRate)}% answer rate — not flagged`,
        };
        ...
        return {
                ...
                color: "text-warning",
                icon: "yellow",
                reason: `${Math.round(answerRate)}% answer rate — may be flagged`,
        };
  } else {
        return {
                label: "Likely Spam",
                color: "text-danger",
                icon: "red",
                reason: `${Math.round(answerRate)}% answer rate — likely marked spam`,
        };
  }
}
...
export default function DashboardPage() {
  ...
  if (!stats) {
        return <div className="p-6 text-sm text-muted">Loading…</div>div>;
  }
  
    const connectRate =
          stats.week.total > 0
            ? Math.round((stats.week.answered / stats.week.total) * 100)
            : 0;
    ...
    return (
          <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto pb-24 md:pb-8">
                <header className="flex items-end justify-between gap-4 flex-wrap">
                        ...
                        <Link href="/dialer" className="btn btn-primary">
                                  <Phone size={16} /> Start dialing
                        </Link>Link>
                </header>header>
                ...
                <section className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-3">
                        <StatCard
                                    ...
                                    sub={`${stats.today.answered} answered`}
                                  />
                        <StatCard
                                    icon={<Timer size={16} />}
                                    label="Talk time today"
                                    value={fmtDuration(stats.today.durationSum)}
                                    sub={
                                                  stats.today.total > 0
                                                    ? `avg ${fmtDuration(
                                                                        Math.round(stats.today.durationSum / stats.today.total),
                                                                      )}/call`
                                                    : "—"
                                    }
                                  />
                        <StatCard
                                    ...
                                    value={`${connectRate}%`}
                                    sub={`${stats.week.answered}/${stats.week.total} calls`}
                                  />
                        <StatCard
                                    icon={<Users size={16} />}
                                    label="Calls this week"
                                    value={stats.week.total.toString()}
                                    sub={`${fmtDuration(stats.week.durationSum)} total talk`}
                                  />
                </section>section>
                ...
                <section className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <div className="lg:col-span-2 card p-5">
                                  <div className="flex items-center justify-between">
                                              <h2 className="text-sm font-medium">Last 7 days</h2>h2>
                                              <span className="text-xs text-muted">calls / answered</span>span>
                                  </div>div>
                                  <DailyTrend rows={stats.dailyTrend} />
                        </div>div>
                
                        <div className="card p-5">
                                  <h2 className="text-sm font-medium mb-3">Dispositions (7d)</h2>h2>
                          {dispTotal === 0 ? (
                        <p className="text-xs text-muted">
                                      No dispositions yet. Tag your calls after hangup.
                        </p>p>
                      ) : (
                                          ...
                                          return (
                                                              <div key={key}>
                                                                                  <div className="flex justify-between text-xs">
                                                                                                        <span>{DISP_LABELS[key] ?? key.replace(/_/g, " ")}</span>span>
                                                                                                        <span className="text-muted tabular-nums">{d.count}</span>span>
                                                                                    </div>div>
                                                                                  <div className="mt-1 h-1.5 bg-elevated rounded-full overflow-hidden">
                                                                                                        <div
                                                                                                                                  className={`h-full ${DISP_COLORS[key] ?? "bg-accent"}`}
                                                                                                                                  style={{ width: `${pct}%` }}
                                                                                                                                />
                                                                                    </div>div>
                                                              </div>div>
            ...
            {/* Number Health Section */}
                <section className="mt-6 card p-5">
                        <div className="flex items-center gap-2 mb-1">
                                  <Activity size={15} className="text-accent" />
                                  <h2 className="text-sm font-medium">Number Health</h2>h2>
                                  <span className="text-xs text-muted ml-1">— based on your answer rates</span>span>
                        </div>div>
                        <p className="text-xs text-muted mb-4">
                                  Low answer rate = likely flagged as spam by carriers. Green ≥35% · Yellow 15–34% · Red &lt;15%
                        </p>p>
                  {stats.numberHealth.length === 0 ? (
                      <p className="text-xs text-muted">No outbound calls yet.</p>p>
                    ) : (
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {stats.numberHealth.map((h) => {
                                      ...
                                      const barPct = Math.min(answerRate, 100);
                                      const barColor =
                                                        health.icon === "green"
                                                          ? "bg-success"
                                                          : health.icon === "yellow"
                                                          ...
                                                          {/* Answer rate bar */}
                                                                          <div>
                                                                                              <div className="flex justify-between text-xs text-muted mb-1">
                                                                                                                    <span>Answer rate</span>span>
                                                                                                                    <span className="tabular-nums font-medium">{answerRate}%</span>span>
                                                                                                </div>div>
                                                                                              ...
                                                                                              <p className="text-[11px] text-muted mt-1">{health.reason}</p>p>
                                                                          </div>div>
                                                          ...
                                                          {/* Stats row */}
                                                                          <div className="flex items-center gap-3 text-xs text-muted border-t border-border pt-2">
                                                                                              <span className="flex items-center gap-1">
                                                                                                                    <PhoneCall size={11} />
                                                                                                                    <span className="tabular-nums">{h.total} calls</span>span>
                                                                                                </span>span>
                                                                                              <span className="flex items-center gap-1">
                                                                                                                    <Clock size={11} />
                                                                                                                    <span className="tabular-nums">{fmtDuration(h.avgDuration)} avg</span>span>
                                                                                                </span>span>
                                                                                              <span className="tabular-nums ml-auto">
                                                                                                                    7d: {h.recentAnswered}/{h.recentTotal}
                                                                                                </span>span>
                                                                          </div>div>
                                                        </div>div>
                                                      );
...
function StatCard({
    ...
    return (
          <div className="card p-4">
                ...
                <div className="mt-2 text-2xl font-semibold tabular-nums">{value}</div>div>
                <div className="text-xs text-muted mt-0.5">{sub}</div>div>
          </div>div>
        );
}
...
function QuickLink({ href, label }: { href: string; label: string }) {
    return (
          <Link
                  href={href}
                  className="card p-4 flex items-center justify-between hover:border-muted transition-colors"
                >
...
function DailyTrend({
    ...
    rows: { day: string; count: number; answered: number }[];
}) {
    if (rows.length === 0) {
          return (
                  <p className="text-xs text-muted mt-2">No calls in the last 7 days.</p>p>
                );
    }
                    ...
                    return (
                                <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                                            <div className="w-full flex-1 flex items-end">
                                                          <div
                                                                            className="w-full bg-elevated rounded-t relative overflow-hidden"
                                                                            style={{ height: `${h}%`, minHeight: d.count > 0 ? 4 : 0 }}
                                                                            title={`${d.count} calls, ${d.answered} answered`}
                                                                          >
                                                                          <div
                                                                                              className="absolute bottom-0 left-0 right-0 bg-accent"
                                                                                              style={{ height: `${(ansH / Math.max(h, 1)) * 100}%` }}
                                                                                            />
                                                          </div>div>
                                            </div>div>
                                            <span className="text-[10px] text-muted">{d.label}</span>span>
                                </div>div>
                              );
          })}
          </div>div>
