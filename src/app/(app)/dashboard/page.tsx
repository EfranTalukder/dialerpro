...
function getHealthStatus(h: NumberHealth): {
      ...
      if (answerRate >= 35) {
              return { label: "Healthy", color: "text-success", icon: "green", reason: `${Math.round(answerRate)}% answer rate — not flagged` };
      } else if (answerRate >= 15) {
              return { label: "Watch", color: "text-warning", icon: "yellow", reason: `${Math.round(answerRate)}% answer rate — may be flagged` };
      } else {
              return { label: "Likely Spam", color: "text-danger", icon: "red", reason: `${Math.round(answerRate)}% answer rate — likely marked spam` };
      }
}
      ...
      return (
              <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto pb-24 md:pb-8">
                    <header className="flex items-end justify-between gap-4 flex-wrap">
                            <div>
                                      <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">Dashboard</h1>h1>
                                      <p className="text-sm text-muted mt-1">
                                          {new Date().toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}
                                      </p>p>
                            </div>div>
                            <Link href="/dialer" className="btn btn-primary"><Phone size={16} /> Start dialing</Link>Link>
                    </header>header>
                    ...
                    <section className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-3">
                            ...
                            <StatCard icon={<Timer size={16} />} label="Talk time today" value={fmtDuration(stats.today.durationSum)} sub={stats.today.total > 0 ? `avg ${fmtDuration(Math.round(stats.today.durationSum / stats.today.total))}/call` : "—"} />
                    ...
                    <section className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
                            ...
                            <div className="card p-5">
                                ...
                                {dispTotal === 0 ? (
                              <p className="text-xs text-muted">No dispositions yet. Tag your calls after hangup.</p>p>
                            ) : (
                  ...
                  {/* Number Health */}
                    <section className="mt-6 card p-5">
                            ...
                            <p className="text-xs text-muted mb-4">
                                      Low answer rate = likely flagged as spam. Green ≥35% · Yellow 15–34% · Red &lt;15%
                            </p>p>
                                              ...
                                              return (
                                                                  <div key={h.fromNumber} className="rounded-lg border border-border bg-elevated p-4 space-y-3">
                                                                                    <div className="flex items-start justify-between gap-2">
                                                                                                        <span className="font-mono text-sm font-medium truncate">{fmtPhone(h.fromNumber)}</span>span>
                                                                                    ...
                                                                                    <div className="flex items-center gap-3 text-xs text-muted border-t border-border pt-2">
                                                                                                        <span className="flex items-center gap-1"><PhoneCall size={11} /><span className="tabular-nums">{h.total} calls</span>span></span>span>
                                                                                                        <span className="flex items-center gap-1"><Clock size={11} /><span className="tabular-nums">{fmtDuration(h.avgDuration)} avg</span>span></span>span>
                                                                                                        <span className="tabular-nums ml-auto">7d: {h.recentAnswered}/{h.recentTotal}</span>span>
                                                                                        </div>div>
                                                                  </div>div>
                                                                );
                            })}
...
function StatCard({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: string; sub: string }) {
      return (
              <div className="card p-4">
                    <div className="flex items-center gap-1.5 text-xs uppercase tracking-wider text-muted">{icon}{label}</div>div>
...
function DailyTrend({ rows }: { rows: { day: string; count: number; answered: number }[] }) {
      if (rows.length === 0) return <p className="text-xs text-muted mt-2">No calls in the last 7 days.</p>p>;
