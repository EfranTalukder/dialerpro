...
export async function GET() {
  ...
  // Number health: all-time stats per outbound number
  const numberHealth = await db
      .select({
              fromNumber: calls.fromNumber,
              total: sql<number>`count(*)::int`,
              answered: sql<number>`count(*) filter (where ${calls.answeredAt} is not null)::int`,
              avgDuration: sql<number>`coalesce(avg(${calls.durationSec}) filter (where ${calls.durationSec} > 0), 0)::int`,
              lastUsed: sql<string>`max(${calls.startedAt})`,
              recentTotal: sql<number>`count(*) filter (where ${calls.startedAt} >= ${week.toISOString()})::int`,
              recentAnswered: sql<number>`count(*) filter (where ${calls.answeredAt} is not null and ${calls.startedAt} >= ${week.toISOString()})::int`,
      })
      .from(calls)
      .where(eq(calls.direction, "outbound"))
      .groupBy(calls.fromNumber)
      .orderBy(desc(sql`count(*)`));
